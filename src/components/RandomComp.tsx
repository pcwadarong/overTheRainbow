...
const RandomComp = memo(({ id, baseSize, color, onClick, usedCoordinates }: LetterBtnProps) => {
  ...
  const positionRef = useRef<{ xPosition: number; yPosition: number } | null>(null);

  // 아이콘 크기 설정 (페이지 새로고침 시에만 랜덤 값 적용)
  if (sizeRef.current === null) {
    const variance = baseSize === 300 ? 100 : 50;
    sizeRef.current = baseSize + Math.floor(Math.random() * (variance * 2 + 1)) - variance;
  }
  ...
  return (
    <button onClick={() => onClick(id)} className="absolute" style={{
      ...
      <IconComponent color={color} width={sizeRef.current} height={sizeRef.current} aria-label={`letter #${id}`} />
    </button>
  );
});
